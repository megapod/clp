...
export default function CookieBanner() {
  ...
  useEffect(() => {
    if (!window.localStorage.getItem('cookiesAccepted')) setVisible(true);
  }, []);
  ...
  return (
    <div className="fixed bottom-0 inset-x-0 bg-gray-800 text-white p-4 flex flex-col md:flex-row items-center justify-between gap-4 z-50">
      ...
      <button onClick={accept} className="bg-teal-500 hover:bg-teal-600 text-white px-4 py-2 rounded-md">
        Got it
      </button>
    </div>
  );
